...
begin
  try
    ...
    if not oAcaoAcesso.ChaveExiste(oAcaoAcesso.Chave) then oAcaoAcesso.Inserir;

    for i := 0 to aForm.ComponentCount -1 do
    begin
      if (aForm.Components[i] is TButton) then
      begin
        if TButton(aForm.Components[i]).Tag = 99 then
        begin
          oAcaoAcesso.descricao := 'BOTÃO '
            + StringReplace(TButton(aForm.Components[i]).Caption, '&', '', [rfReplaceAll]);

          oAcaoAcesso.Chave := aForm.Name + '_' + TButton(aForm.Components[i]).Name;
...
begin
  try
    ...
    Qry.SQL.Add('SELECT ID FROM usuarios ');
    Qry.Open;
    ...
    begin
      ...
      begin
        VerificarUsuarioAcao(Qry.FieldByName('ID').AsInteger, QryAcaoAcesso.FieldByName('acaoAcessoId').AsInteger, aConexao);
        QryAcaoAcesso.Next;
      end;
    ...
    if Assigned(Qry) then FreeAndNil(Qry);
    if Assigned(QryAcaoAcesso) then FreeAndNil(QryAcaoAcesso);
  end;
end;

...
begin
  try
    ...
    Try
      ...
      Self.F_usuarioId     := Qry.FieldByName('ID').AsInteger;
...
begin
  try
    ...
    Qry.Connection:=ConexaoDB;
    Qry.SQL.Clear;
    Qry.SQL.Add('SELECT COUNT(ID) AS Qtde '+
                '  FROM Usuarios '+
                ' WHERE Nome =:Nome ');
  ...
  try
    ...
    Try
      ...
      if Qry.FieldByName('ID').AsInteger > 0 then begin
         Result := true;
         F_usuarioId := Qry.FieldByName('ID').AsInteger;
...
begin
  try
    ...
    Qry.SQL.Add('UPDATE usuarios SET senha = :senha WHERE ID = :usuarioId ');
    Qry.ParamByName('usuarioId').AsInteger := Self.F_usuarioId;
    Qry.ParamByName('senha').AsString := Self.F_Senha;
